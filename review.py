...
ENTITY_TYPES = ["Filename", "Linenumber", "Input", "Function", "Object", "Method", "Tainted", "Untainted", "Sink"]
...
def parse_plain_text_nodes(response, filename="Unknown"):
    """
    ...
    for line_num, line in enumerate(lines, start=1):  # Adding line number from the file
        try:
            if "-" in line:
                entity_id = line.split(".")[0].strip()  # Line number as unique ID
                ...
                nodes.append({
                    ...
                    "line_number": line_num,  # Add the line number here
...
def clean_node_format(node):
    ...
    node['line_number'] = node.get('line_number') if node.get('line_number') is not None else "Unknown"
...
def parse_plain_text_edges(response, filename="Unknown"):
    ...
    for match in matches:
        ...
        if file_name == "Relationships":
            file_name = filename  # If filename is incorrectly set to 'Relationships', replace it with the default or passed filename

        # Debugging print to ensure filename and line number are correctly parsed
        ...
        edges.append({
            ...
            "line_number": line_number.strip(),  # Line number remains as captured
...
def extract_graph_data(grag, filename="Unknown"):
    ...
    try:
        ...
        edge_response = grag.query("List all relationships between entities adding line number and filename to nodes")
...
def push_to_neo4j(nodes, edges):
    ...
    try:
        with driver.session() as session:
            ...
            for node in nodes:
                query = """
                MERGE (n:Entity {id: $id, type: $type, category: $category, line_number: $line_number, filename: $filename})
                """
                session.run(query, id=node.get("id"), 
                            type=node.get("type"), 
                            category=node.get("category"), 
                            line_number=node.get("line_number", "Unknown"),  # Default to 'Unknown' if line_number is None
            ...
            for edge in edges:
                ...
                MERGE (a)-[:RELATED_TO {type: $type, line_number: $line_number, filename: $filename}]->(b)
                """
                session.run(query, source=edge.get("source"), 
                            target=edge.get("target"), 
                            type=edge.get("type"),
                            line_number=edge.get("line_number", "Unknown"),  # Default to 'Unknown' if line_number is None
...
if __name__ == "__main__":
    ...
    directory_path = "./code/"  # Update this to your directory
    ...
    nodes, edges = extract_graph_data(grag, filename="Unknown")  # Pass filename during extraction
